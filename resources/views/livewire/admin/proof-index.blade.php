<div>
    <header class="mb-8 flex items-end justify-between gap-4">
        ...
        <a href="@php echo route("admin.competitions.index"); @endphp" class="px-5 py-2 rounded-card bg-ignite text-paper font-display font-bold">Vincular a competição</a>
  </header>
        ...
        <ul class="space-y-4">
            @foreach ($this->proofs as $p)
                <li class="bg-white rounded-card border border-rule p-5 shadow-card flex items-baseline gap-4">
                    ...
                    <a href="@php echo route("admin.proofs.edit", $p->id); @endphp" class="text-ignite font-display font-semibold">Editar</a>
               </li>
            @endforeach
      </ul>
    @endif
